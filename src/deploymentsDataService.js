...
class DeploymentsDataHandler {
  ...
  getDeploymentsDashboard() {
    return this.getAllDeployments().then(
      result => new DeployementsOverviewModel(result)
    );
  }

  getDeploymentDetails(id) {
    return this.getAllDeployments().then(
      result => result.record.filter(dep => dep.id === id)[0]
    );
  }

  getDeploymentList() {
    return this.getAllDeployments().then(
      result => new DeploymentsListModel(result)
    );
  }
  ...
  getAllDeployments() {
    ...
    return this.deploymentsCache
      ? Promise.resolve(this.deploymentsCache)
      : loadAllDeployments().then(data => {
