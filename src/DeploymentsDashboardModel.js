...
class DeploymentsDashboardModel {
  ...
  constructor(data) {
    ...
    this.totalHealthyDeployments = data.totalCount - this.unhealthyDeployments.length;
    this.totalKibana = data.record.filter(dep => dep.kibana.enabled).length;
    this.totalMonitoring = data.record.filter(dep => dep.monitoring.enabled).length;
...
function getUnhealtyDeployments(deployments) {
  return deployments.filter(deployment => !deployment.healthy).map(dep => new UnhealthyDeploymentModel(dep));
}
