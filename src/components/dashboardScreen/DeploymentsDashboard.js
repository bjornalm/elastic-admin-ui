...
class DeploymentsDashboard extends Component {
  ...
  componentDidMount() {
    ...
    dataService.getDeploymentsDashboard().then((dashboardModel) => { 
  ...
  renderUnhealthyDeployments() {    
    return (<EuiPanel paddingSize="l" hasShadow>
      <EuiPageContentHeader>
        <EuiPageContentHeaderSection>
        <EuiTitle>            
            <h2><EuiIcon size="xxl" type={`alert`} color="danger" /> Unhealthy Deployments</h2>
          </EuiTitle>
        </EuiPageContentHeaderSection>
      </EuiPageContentHeader>
  ...
  renderStats() {
    const unhealthy = this.state.unhealthyDeployments && this.state.unhealthyDeployments.length;
    return (
