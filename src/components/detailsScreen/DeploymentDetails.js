...
class DeploymentDetails extends Component {
  ...
  componentDidMount() {
    ...
    dataService.getDeploymentDetails(deploymentId).then(
      details => {
        this.setState({ ...this.state, loading: false, details: details });
      },
      err => {
        console.error(err);
        this.setState({
        ...
        <EuiFlexItem>
          <div>
            <EuiIcon
              size="xxl"
              type={`dot`}
              color={details.healthy ? "#490" : "danger"}
            />
