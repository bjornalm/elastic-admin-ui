...
class DetailsCardGroup extends Component {
  ...
  render() {
    ...
    const premium = user.isPremium ? 
        <EuiBadge color="primary" iconType="starFilled" iconSide="right"> Premium </EuiBadge> : null;

    return (
      <EuiFlexGroup gutterSize="l">          
        <EuiFlexItem>
          <EuiCard
            icon={<EuiIcon size="xxl" type={`online`} color={isStopped ? 'default' : 'secondary'}/>}
        ...
        <EuiFlexItem>
          <EuiCard
            icon={<EuiIcon size="xxl" type={`visGauge`} color={monitoring.enabled ? 'secondary' : 'default'}/>}
            title={monitoring.enabled ? 'Monitoring' : 'No Monitoring'}
            description={ 
              <span> 
                Deployment has monitoring {monitoring.enabled ? 'enabled' : 'disabled'}
