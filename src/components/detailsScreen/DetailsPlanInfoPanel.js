...
class DetailsPlanInfoPanel extends Component {
  ...
  render() {
    ...
    const planList = createDescriptionList(
      ['isActive', 'isPending', 'instanceCapacity', 'instanceCount', 'availabilityZones'], plan);

    const test = plan.configurationSteps.map((step, index) => (
        <EuiListGroupItem key={index} size="s" label={
          <span>
          <b>Time:</b> {step.time} 
    ...
    return (
      <EuiPanel>
        <EuiFlexGroup gutterSize="l">
          <EuiFlexItem>
            <DetailsHealthInfo name={`Plan (${plan.version})`} healthy={plan.healthy} list={planList}/>
          </EuiFlexItem>
          <EuiFlexItem>
            <EuiSpacer />
            <EuiAccordion id="accordion1" buttonContent={<EuiText size="xs">Show configuration Steps</EuiText>}>
              <EuiSpacer />
